...
class PLCController:
    ...
    def set_value(self):
        ...
        self._last_connect_params = {}  # Lưu params để dùng lại khi reconnect
    ...
    def on_PLC_connect(self, params):
        ...
        # Lưu params để dùng lại khi reconnect
    ...
    def read_M_continuos(self):
        ...
        # Reconnect logic
        RECONNECT_THRESHOLD = 50  # Số lần fail liên tiếp trước reconnect
        fail_count = 0
        ...
        if self.protocol is not None:
            while self.thread_read_PLC:
                ...
                if read_value is not None and not read_value.isError():
                    fail_count = 0  # Reset fail counter khi read thành công
                    ...
                    if fail_count >= RECONNECT_THRESHOLD:
                        ...
                        if self._last_connect_params:
                            ...
                            if self.protocol.connect(**self._last_connect_params):
                                fail_count = 0  # Reset nếu reconnect thành công
                            else:
                                # Reconnect thất bại, emit signal
                                signal.PLC_disconnected.emit()
                                self.PLC_status = False
                                break  # Thoát vòng lặp, chờ reconnect thủ công
                        else:
                            break
                    time.sleep(0.002)
