...
from PyQt5.QtWidgets import (QGraphicsScene, QMessageBox, QGraphicsRectItem, QGraphicsPolygonItem,
...
class ReferenceImage(QGraphicsScene):
    ...
    def set_event(self):
        ...
        signal.update_img_size.connect(self.on_update_roi_rect_list)
    ...
    # Excecute=======================================================================
    @catch_errors
    def on_show_grapped_image(self, img, time_receive = 0):
            if not initialize_secure_dongle():
                signal.show_error_message_main.emit("[on_show_grapped_image] License key not found!")
            ...
            if not self.GUI.live_camera_status and start_time == 0:
            ...
            #print("thoi gian xu ly OCR", processing_time)
            ...
            if self.GUI.real_time_status:
                ...
                if sep in base:
                    ...
                    if len(parts) == 2:
                        ...
                        rev_variants.add(right[::-1] + left[-1::-1][0] + sep + left[::-1][1:])
                        rev_variants.add(right[::-1][:-1] + sep + right[::-1][-1] + left[::-1])

                # Compile pattern thuận 1 lần duy nhất
                pattern_forward = re.compile(rf"(?:(?<=^)|(?<=[-_])){re.escape(base)}(?:(?=$)|(?=[-_]))")

                # Compile tất cả pattern đảo 1 lần duy nhất
                patterns_reverse = [re.compile(rf"(?:(?<=^)|(?<=[-_])){re.escape(variant)}(?:(?=$)|(?=[-_]))")
                ...
                self.draw_ROI(pixmap, roi_ORC_list, image_width=width, OCR_text_list=OCR_text_list)
            ...
            processing_time_ms = f"{processing_time:.1f} ms" # Hiển thị UI
            # self.GUI.label_cycle_time.setText(processing_time_ms)
            if start_time != 0:
                # Nếu đang live thì tính fps chụp thì reset 
                self.time_update += elapsed_time
                self.fps_count += 1
            else:
                self.time_update = 0
                self.fps_count = 0
            
            #Nếu đã trôi qua 1 giây, tính toán FPS và đặt lại biến
            if self.time_update>= 1.0:
                fps = self.fps_count / int(self.time_update)
                fps = f"{fps:.0f}" # Hiển thị UI
                # print(fps, processing_time_ms)
                self.GUI.label_fps.setText(str(fps))
                self.GUI.label_cycle_time.setText(processing_time_ms)
                self.time_update=0
                self.fps_count =0
    ...
    def start_thread_OCR(self, status):
            if status:
                ...
                self.thread_OCR = threading.Thread(target=self.OCR_detect_continuous,
                                                   daemon=True)
    ...
    def on_update_roi_rect_list(self):
        # Update default ROI
        self.roi_rect_list = [[self.GUI.ROIx1-self.GUI.offset_x, self.GUI.ROIy1-self.GUI.offset_y, 300, 440, 0], # x, y, w, h, color
                            [self.GUI.ROIx2-self.GUI.offset_x, self.GUI.ROIy2-self.GUI.offset_y, 300, 440, 0],
                            [self.GUI.ROIx3-self.GUI.offset_x, self.GUI.ROIy3-self.GUI.offset_y, 300, 440, 0],
                            [self.GUI.ROIx4-self.GUI.offset_x, self.GUI.ROIy4-self.GUI.offset_y, 300, 440, 0],
                            [self.GUI.ROIx5-self.GUI.offset_x, self.GUI.ROIy5-self.GUI.offset_y, 300, 440, 0]
                            ]
    ...
    def crop_ROI(self, roi_rect_list):
        if self.img_crop is not None and self.img_crop.size > 0:
            for idx, roi_rect in enumerate(roi_rect_list):
                ...
                now = datetime.now()
                current_time = now.strftime("%Y_%m_%d_%H_%M_%S") + f"_{now.microsecond // 1000:03d}"
        ...
        for roi_rect in self.roi_rect_list:
            ...
            if self.GUI.real_time_status:
                ...
                with self._inference_lock:
                    result = self.OCR_DEEP_LEARNING_TOOL.Prediction_OCR_None_Img(
                                img_ocr,
                                self.model_OCR,
    ...
    def save_pixmap_image(self, pixmap, result):
        ...
        image_arr = np.array(ptr).reshape((qimg_rgba.height(), qimg_rgba.width(), 4)) # lay ra anh numpy
        now = datetime.now()
        current_time = now.strftime("%Y_%m_%d_%H_%M_%S") + f"_{now.microsecond // 1000:03d}"
        img_name = f"{result}_{self.GUI.current_product}_{current_time}.bmp"
        os.makedirs(f"{self.drive}/DRB Metalcore Text Result/{now.strftime('%d_%m_%Y')}/{result}", exist_ok=True)
        # save_path_1 = os.path.join(f"{self.drive}/DRB Metalcore Text Result", img_name)
        save_path_2 = os.path.join(f"{self.drive}/DRB Metalcore Text Result/{now.strftime('%d_%m_%Y')}/{result}", img_name)
