...
class Authentication(QMainWindow):
    ...
    def On_Save(self):
        try:
            ...
            new_password = self.Le_Password_Cre.text().strip()
            ...
            updates = {"FullName": full_name, "Department": department,
                       "No_id": no_id, "Role": role_group, "Active": status}

            # Chỉ đổi password khi user nhập mới (để trống = giữ nguyên)
            if new_password:
                if CheckPasswordMessage(new_password) is not True:
                    QMessageBox.warning(self, "Fail", CheckPasswordMessage(new_password))
                    return
                updates["PasswordHash"] = hash_password(new_password)

            User.update(column="UserName", value=user_name, updates=updates)

            LoginAudit.insert({"UserID": self.current_user["UserID"], "UserName": self.current_user["UserName"], "EventType": f"Change User {user_name} Credentials"})
            AuditTrial.insert({"UserName": self.current_user["UserName"], "Action": "Change User Credentials"})
    ...
    def On_LoadUserInfo(self):
        try:
            ...
            if info:
                ...
                self.Le_Password_Cre.clear()  # Không hiển thị hash — để trống nếu không đổi
    ...
    def On_AddUser(self):
        try:
            ...
            if CheckPasswordMessage(password_hash) is not True:
                ...
                return
            User.insert({"UserName": user_name, "FullName": full_name,
                         "PasswordHash": hash_password(password_hash),
                         "Department": department, "No_id": no_id, "Role": role_group})
