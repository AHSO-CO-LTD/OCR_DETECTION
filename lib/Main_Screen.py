...
class MainScreen(QMainWindow):
    # ===== Constants =====
    AUTO_STOP_TIMEOUT_MS = 2 * 60 * 1000  # 2 minutes auto-stop
    EXPECTED_QUANTITY = 5  # Số lượng mẫu cần chụp
    NG_FRAME_THRESHOLD = 30  # Ngưỡng NG frames cho phép
    ...
    def set_value(self):
        ...
        self.ng_frame = 0
        self.button_stylesheet_off = self.BUTTON_STYLE_OFF
        self.button_stylesheet_on = self.BUTTON_STYLE_ON
    ...
    def set_timer(self):
        ...
        self.stop_timer.start(self.AUTO_STOP_TIMEOUT_MS)
    ...
    def on_count(self, quantity, result, ok_count, ng_count):
        ...
        if result and self.quantity == self.EXPECTED_QUANTITY:
        ...
        elif self.quantity != self.EXPECTED_QUANTITY or self.ng_frame < self.NG_FRAME_THRESHOLD:
        ...
        if self.quantity == self.EXPECTED_QUANTITY and ng_count != 0:
